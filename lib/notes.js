...
class Notes {
  ...
  async execute() {
    ...
    if (this.data.updateID) { // updates a note with -u/--update with the selected ID, along with passed text, category parameters to change
      ...
      console.log(updatedNote ? `Updated Note ${updatedNote}` : `Note with ID:${this.data.updateID} non existent`);
    };
  ...
  help() {
    console.log(
      ...
      --update selects the note by id, and updates its fields using the following format "id, <note>, <category>`
    );
  }
}
